...
extension UILabel {
    ...
    /// - Parameters:
    ///   - maxFontSize: the max font size available
    ///   - minFontScale: the min font scale that the font will have
    ///   - rectSize: rect size where the label must fit
    public func fontSizeToFit(
        ...
        let newFontSize = fontSizeThatFits(
            text: unwrappedText,
    ...
    public func fontSizeThatFits(
        ...
        let calculatedFontSize = binarySearch(
            string: string,
...
extension UILabel {
    ...
    private func currentAttributedStringAttributes() -> [NSAttributedString.Key: Any] {
        ...
        attributedText?.enumerateAttributes(
            in: NSRange(0..<(text?.count ?? 0)),
            options: .longestEffectiveRangeNotRequired,
            using: { attributes, _, _ in
